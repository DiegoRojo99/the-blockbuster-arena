...
const Index = () => {
  ...
  if (currentGame === 'cast') {
    return <CastGame onBack={() => setCurrentGame('home')} />;
  }

  if (currentGame === 'grid') {
    return <GridGame onBack={() => setCurrentGame('home')} />;
  }
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {games.map((game, index) => (
              <GameCard
                ...
                className={`animate-fade-in ${index === 1 ? '[animation-delay:0.2s]' : ''}`}
              />
            ))}
          </div>
        </div>
